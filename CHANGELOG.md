...
### Added
- **豆包 Doubao 多模态翻译** (`doubao-seed-1-8-251428` via ARK)
  - 图片+中文标题 → 俄语标题/描述/关键词
  - 简化中文提示词，固定话术发送
- 翻译 API 支持图片上传（豆包优先，DeepSeek 兜底）
- 关键词自动格式化（去除下划线，Ozon 格式 `#набордлявышивки`）

### Changed
- 豆包提示词简化为两句话：
  - 翻译标题：`你好豆包，请根据图片和中文标题，翻译成适合在OZON上架的俄语标题。`
  - 生成描述：`你好豆包，请根据图片和中文标题，翻译成适合在OZON上架的俄语关键词和商品简介。关键词要以#号隔开。`
- 不再要求关键词数量，越多越好
- 图片预览缩小为 80x80 固定尺寸

### Fixed
- 产品表单图片预览不撑开页面
- `api_generate_description` Path 导入修复
- 服务器进程残留导致旧代码运行
...
### Added
- **豆包 Doubao 多模态翻译** (`doubao-seed-1-8-251228`)
  - 图片+中文标题 → 俄语标题/描述/关键词
  - 看图识别，不再捏造花卉品种
  - 自动回退 DeepSeek（无图片时）
- 双栏产品表单：🟠供应商端 / 🔵Ozon输出端
- AI 操作按钮组：🌐翻译标题 / 📝生成描述+关键词 / 📐图片转3:4
- 商品表单实时联动：`原价 = 售价 ÷ 60%`
- 豆包 Ozon 优化提示词 — 描述基于图片，禁止虚构

### Fixed
- 标题翻译语句颠倒 — `рисунок с X` 语法修正
- `рамка/фоторамка/полуфабрикат/оптовый` 等16个C端禁用词彻底屏蔽
- 中文标题过滤：批发/货源/相框/摆件/半成品/厂家/代理
- 描述 `<p>` 标签自动清除
- 定价计算器读取 DOM 值修复
- 增值税/货币/品牌改为隐藏字段（默认0/CNY/无品牌）

### Changed
- DeepSeek 翻译 → 豆包多模态为主（有图片时）
- 标题风格：简洁关键词式 `Набор для вышивки + 图案 + 29 см + 技法 + 受众`
- 描述格式：纯文本，无HTML标签
